...
namespace AmivoiceWatcher
{
    ...
    class Globals
    {
        public static string appName = "Amivoice Watcher";
        ...
        public static void Configuration_SetValueFromSetFile()
        {
            ...
            if (!Int32.TryParse(Configuration.configuration["notification.client.max"], out Globals.notification_client_max))
            {
            ...
            if (!Int32.TryParse(Configuration.configuration["notification.duration"], out Globals.notification_duration))
            {
        ...
        {
            string[] paths = MyPath.CleanAllDirectoryPath;
        ...
        public static void CreateAllDirectoryAndFiles()
        {
            try
            {
                string[] paths = MyPath.CreateAllDirectoryAndFilesPath;
                ...
                var SourcePath = MyPath.CreateAllDirectoryAndFilesPathCopyFrom;
                var DestinationPath = MyPath.CreateAllDirectoryAndFilesPathCopyTo;
            ...
            {
                Globals.log.Debug("Unable to CreateAllDirectoryAndFiles()");
        ...
        public class functions
        {
            ...
            public static HttpPostRequestReturnCode HttpPostRequestDictionary(string path, Dictionary<string, string> dictIn, int timeoutIn = 5000)
            {
                try
                {
                    using (var client = new WebClientWithAwesomeTimeouts { Timeout = System.TimeSpan.FromMilliseconds(timeoutIn) })
                    {
                        ...
                        client.Headers.Add(@"User-Agent: Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/51.0.2704.106 Safari/537.36");
            ...
            public static HttpPostRequestReturnCode HttpPostRequestFileUpload(string path, string filepath, int timeoutIn = 100000)
            {
                try
                {
                    using (var client = new WebClientWithAwesomeTimeouts { Timeout = System.TimeSpan.FromMilliseconds(timeoutIn) })
                    {
                        ...
                        //client.Headers.Add("User-Agent: Mozilla/5.0 (compatible; MSIE 9.0; Windows NT 6.1; WOW64; Trident/5.0)");
                        client.Headers.Add(@"Mozilla/5.0 (Windows NT 6.1) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/62.0.3202.89 Safari/537.36");

                        client.Headers.Add("Accept-Language", "en-US,en;q=0.9,th;q=0.8");
            ...
            public static string HtmlWithAbsolutePaths(string tempPathString, string html)
            {
                ...
                string appDirectory = MyPath.PathLocalAppData;
            ...
            {
                string pathOut = Path.Combine(MyPath.PathLocalAppData, "Template", pathIn);
                return pathOut;
            }
            ...
            public static string Json_toJsonObj(object objIn)
            {
                return JsonConvert.SerializeObject(objIn);
            }

            public static void TrySetValueFromDictionary(ref string varOut, Dictionary<string, string> dictIn, string key)
            {
                if (dictIn.ContainsKey(key))
                {
                    varOut = dictIn[key];
                }
            }

            public static void TrySetIntValueFromDictionary( ref int varOut, Dictionary<string, string> dictIn, string key)
            {
                if (dictIn.ContainsKey(key))
                {
                    Int32.TryParse(dictIn[key], out varOut);
                }
            }

            public static string Json_toJsonFormatted(object objIn)
            {
                return JsonConvert.SerializeObject(objIn, Formatting.Indented);
            }
...
//                try
//                {
//                    var htmlString = MyPath.htmlStringTemplateSuper;

//#if DEBUG
//                    htmlString = File.ReadAllText(@"\\VBOXSVR\SharedLocal\AMIVOICE\projects\AmivoiceWatcher\amivoice_watcher\AmiVoiceWatcher2\AmivoiceWatcherDotNet\AmivoiceWatcherDotNet\LocalAppData\Template\SuperNotificationTemplate.html");
//#endif
...
//                }
        ...
        public class GUI
        {
            public static string ApplicationName = "Amivoice Assistant";
        }

        public class Notifications
        {
            public class URL {
                //public static string watchercli = Configuration.configuration["aohs.baseurl"] + @"/watchercli/"+Globals.myComputerInfo.UserName+"/notification";
                //public static string baseurl = Configuration.configuration["aohs.baseurl"] + "/"  ;

                public static string Watchercli()
                {
                    return Configuration.configuration["aohs.baseurl"] + @"/watchercli/" + Globals.myComputerInfo.UserName + "/notification";
                }

                public static string Baseurl()
                {
                    return Configuration.configuration["aohs.baseurl"] + "/";
                }
            ...
            public static void PopupWelcomeMessage()
            {
                try
                {
                    ...
                    var jsonMsg = Globals.functions.Json_toJsonObj(dictMsg);
                    AmivoiceWatcher.NotificationPanel.Popup(jsonMsg, duration, animationMethod, animationDirection);
    ...
    //    public string error = Path.Combine(MyPath.PathLocalAppData, "Template", "image", "Button-Close-icon.png");
    //    public string notice = Path.Combine(MyPath.PathLocalAppData, "Template", "image", "Button-Info-icon.png");
    //    public string success = Path.Combine(MyPath.PathLocalAppData, "Template", "image", "ok-icon.png");
    //    public string warning = Path.Combine(MyPath.PathLocalAppData, "Template", "image", "Button-Warning-icon.png");
    //    public string question = Path.Combine(MyPath.PathLocalAppData, "Template", "image", "Help-icon.png");
